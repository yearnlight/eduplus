<template>
  ...
  >
    <el-form :model="billData" label-width="120px">
      ...
      <el-form-item
        v-if="schoolAttrType == 'auth_userinfo'"
        label="家长手机："
        >{{ billData.authValue }}</el-form-item
      >
      <el-form-item
        v-if="schoolAttrType == 'auth_studentno'"
        label="学生学号："
        >{{ billData.authValue }}</el-form-item
      >
      <el-form-item
        v-if="schoolAttrType == 'auth_identity'"
        label="学生身份证号："
        >{{ billData.authValue }}</el-form-item
      >
      ...
      <el-form-item label="账单详情：">
        <el-table
          v-if="
            billData.planChargeDetailJoin &&
              billData.planChargeDetailJoin.length
          "
          :data="billData.planChargeDetailJoin"
          tooltip-effect="dark"
          style="width: 300px"
          border
          :size="'medium'"
          ...
          <el-table-column prop="chaName" label="名称" width="180">
            <template slot-scope="scope">
              <span class="check-box-span"
                ><el-checkbox
                  v-if="scope.row.needPay == 2"
                  disabled
                  :checked="scope.row.toPay==1"
                ></el-checkbox></span
              >{{ scope.row.chaName }}
            </template>
          </el-table-column>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="checkBillDialogVisible = false"
        >确 定</el-button
      >
    </div>
  </el-dialog>
